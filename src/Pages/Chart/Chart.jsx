        ...
        <Row>
              ...
              chartItems.map((ele) => (
                <div className="chartItem mb-5" key={ele.id}>
                  ...
                  <div className="chartItem_info flex_center flex-column">
                    ...
                    <div className="price">
                      <span className="currentPrice">EGP {ele.price}</span>
                      {ele.oldprice && (
                        <span className="oldprice">EGP {ele.oldprice}</span>
                      )}
                    </div>
                    <ul className="chartItem_control">
                      <li onClick={() => dispatch(decreaseItemQuantity(ele))}>
                        -
                      </li>
                      <li className="num">{ele.num}</li>
                      <li onClick={() => dispatch(increaseItemQuantity(ele))}>
                        +
                      </li>
                    </ul>
                    <p className="chartItem_total">
                      <span className="chartItem_total_name">Total: </span>
                      <span className="total">EGP {ele.num * ele.price}</span>
                    </p>
                    <div className="confirm d-flex gap-2">
                      <button
                        className="Remove"
                        onClick={() => dispatch(deleteChartItem(ele))}
                      >
                        Remove
                      </button>
                      <Link className="buy custom_button" to="/Login">
                        Buy
                      </Link>
                    </div>
                  </div>
          ...
          >
            <div className="CartSummary">
              ...
              {chartItems &&
                chartItems.map((ele) => (
                  <p className="CartSummary_item" key={ele.id}>
                    <span className="CartSummary_item_name">{ele.name}:</span>
                    <span className="CartSummary_item_price">
                      EGP {ele.price * ele.num}
                    </span>
                  </p>
                ))}
              <p className="CartSummary_total">
                <span className="CartSummary_total_name">Total:</span>
                <span className="CartSummary_total_number">
                  EGP{" "}
                  {chartItems.length > 0
                    ? totalPrice.reduce((acc, current) => acc + current)
                    : 0}
                </span>
              </p>
            </div>
